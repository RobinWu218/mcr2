...
import architectures.resnet_resadpt as models
...
import config_task
import residual_adapters.utils_pytorch as utils_pytorch
import residual_adapters.sgd as sgd
...
parser.add_argument('--savedir', default='/scratch/shared/nfs1/srebuffi/exp/dem_learning/tmp/', help='Save folder')
...
args.ckpdir = args.savedir + '/checkpoint/'
args.svdir  = args.savedir + '/results/'
...
trainset = tf.load_trainset(args.dataset[0], transform_train, train=True, path=args.datadir)
testset = tf.load_trainset(args.dataset[0], transform_test, train=False, path=args.datadir)
...
net = models.resnet18(args.num_classes)
store_data = []
...
for name, m in net_old.named_modules():
    if isinstance(m, nn.BatchNorm2d) and 'bn' in name:#'bns.' in name:
        names.append(name)
        store_data.append(m.weight.data)
...
for id_task in range(len(args.num_classes)):
    element = 0
    for name, m in net.named_modules():
        if isinstance(m, nn.BatchNorm2d) and 'bns.'+str(id_task) in name and not re.search(condition_bn,name) and not 'end' in name:
