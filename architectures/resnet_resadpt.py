...
class ResNet(nn.Module):
    def __init__(self, block, nblocks, num_classes=[10]):
        ...
        self.conv1 = conv_task(3,int(64*factor), 1, nb_tasks) 
        self.layer1 = self._make_layer(blocks[0], int(64*factor), nblocks[0], stride=1, nb_tasks=nb_tasks)
        ...
        self.linears = nn.ModuleList([nn.Linear(int(512*factor), num_classes[i]) for i in range(nb_tasks)])         
