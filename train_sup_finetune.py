...
# print("Use MCR objective to finetune models from experts trained with expansion term only")
print("Use MCR objective to finetune models from experts trained with full term")
for ds_name in os.listdir(args.data_dir)[7:]:
    if ds_name == "omniglot":
        continue
    print("Start fintune on dataset:{}".format(ds_name))
    for source_name in os.listdir(args.data_dir):
        if source_name == ds_name:
            continue
        if source_name == "omniglot":
            continue
        print("Use model pretrained on dataset:{}".format(source_name))

        ## Pipelines Setup
        model_dir = os.path.join(args.save_dir,ds_name,
                    'sup_finetune_{}+{}_{}_epo{}_bs{}_lr{}_mom{}_wd{}_gam1{}_gam2{}_eps{}_lcr{}{}'.format(
                            args.arch, args.fd, source_name, args.epo, args.bs, args.lr, args.mom, 
