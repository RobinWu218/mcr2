...
parser.add_argument('--data', type=str, default='cifar10', help='dataset for training (default: CIFAR10)')
...
pretrain_mode = args.model_dir.split("/")[2]
print("Start Evaluation on {} pretrained networks".format(pretrain_mode))
...
res = np.zeros(len(dataset_list),dtype=dict)
target_ds_name = args.data
...
for i, source_ds_name in enumerate(dataset_list):
    ...
    stats_dict = {}
    stats_dict['backbone'] = source_ds_name
    pretrained_model_dir = args.model_dir.split("{}")[0]+target_ds_name+args.model_dir.split("{}")[1]+source_ds_name+args.model_dir.split("{}")[2]
    ## load model
...
np.save("fintune_result_{}_{}".format(target_ds_name,pretrain_mode),res)
